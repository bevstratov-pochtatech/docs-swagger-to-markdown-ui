...
module.exports = (responses) => {
  ...
  Object.keys(responses).forEach((responseCode) => {
    ...
    if ("examples" in response) {
      description += Object.entries(response.examples)
        .map(([contentType, example]) => {
          ...
          return `<br><br>**Пример ответа** (*${contentType}*):<br><pre>${formattedExample}</pre>`;
        })
        .join("");
    }
  ...
  res.unshift(`| ---- | ----------- |${schemas ? " ------ |" : ""}`);
  res.unshift(`| Код | Описание |${schemas ? " Схема данных |" : ""}`);
  res.unshift("##### Ответы сервера\n");
