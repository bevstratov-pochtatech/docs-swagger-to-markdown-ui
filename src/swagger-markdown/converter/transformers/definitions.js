  ...
  Object.keys(definition.properties).forEach((propName) => {
    ...
    if ("enum" in prop) {
      const enumValues = prop.enum
        ...
        .join(", ");
      descriptionParts.push(`_Перечисление:_ ${enumValues}`);
    }
    if ("example" in prop) {
      descriptionParts.push(`_Пример:_ \`${JSON.stringify(prop.example)}\``);
    }
    const descriptionCell = descriptionParts.join("<br>");
    const requiredCell = inArray(propName, required) ? "**Да**" : "Нет";
    res.push(
...
const processDefinition = (name, definition) => {
  ...
  res.push("| Наименование | Тип | Описание | Обязательное |");
  res.push("| ---- | ---- | ----------- | -------- |");
